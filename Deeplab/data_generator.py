...
3.1. ADE20K with SPADE generated images

We generated images from ADE using SPADE. After a SPADE training over the ADE20K
dataset, we generated new images from both the train and validation set, leading
to 22K new images. We add those building new splits.

4. Guitars dataset

The Guitars dataset has been provided by Zhinhan and Tono from EventLab. By now,
it includes images and labels for 2 semantic classes manually checked on pictures
of very diverse contexts.

5. Guitarfold

The Guitar dataset reorganized for K-fold cross validation. Data are formatted as
960x720 size and no color stardardization.
...
_ADE20K_INFORMATION = DatasetDescriptor(
    splits_to_sizes={
        ...
        'train_spade': 42420, # num of ADE + 22420 SPADE generated images
        'val_spade': 2000, # same as before
    },
    num_classes=151,
    ignore_label=0,
)

_GUITAR_INFORMATION = DatasetDescriptor(
	splits_to_sizes={
		'trainval': 2199, # original trainval
    'train': 1759, # 80% of original trainval
    'eval': 440, # 20% of original trainval
		'trainval_aug': 10330, # augmented (crop size + color std)
    'train_aug': 8264, # 80% of the previous set
    'eval_aug': 2067, # remaining (one from ....?)
    'vis': 768, # video frames for test, NO labels
    'spade_aug_train': 14875, # train_aug split + spade generated
    'spade_aug_eval': 3720, # eval_aug split + spade generated
	},
	num_classes=2,
	ignore_label=255, # class label should be between 0 and num_classes excluded
)	

_GUITARFOLD_INFORMATION = DatasetDescriptor(
  splits_to_sizes={ # all recent folds must be written by hand: ALWAYS CHECK!
    'fold_1_train': 3465, 'fold_1_eval': 385,
    'fold_2_train': 3465, 'fold_2_eval': 385,
    'fold_3_train': 3465, 'fold_3_eval': 385,
    'fold_4_train': 3465, 'fold_4_eval': 385,
    'fold_5_train': 3465, 'fold_5_eval': 385,
    'fold_6_train': 3465, 'fold_6_eval': 385,
    'fold_7_train': 3465, 'fold_7_eval': 385,
    'fold_8_train': 3465, 'fold_8_eval': 385,
    'fold_9_train': 3465, 'fold_9_eval': 385,
    'fold_10_train': 3465, 'fold_10_eval': 385,
  },
  num_classes=2,
  ignore_label=255,
)
...
_DATASETS_INFORMATION = {
    ...
    'guitars': _GUITAR_INFORMATION,	
    'guitarfold': _GUITARFOLD_INFORMATION,
    'motive': _MOTIVE_INFORMATION,
}
